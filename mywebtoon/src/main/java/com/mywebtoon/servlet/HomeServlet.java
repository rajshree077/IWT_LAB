...
public class HomeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        List<Comic> recentComics = new ArrayList<>();
        
        try (Connection conn = DBConnection.getConnection()) {
            // Only show APPROVED comics on home
            String sql = "SELECT c.*, u.username as author_name FROM comics c JOIN users u ON c.creator_id = u.id WHERE c.status = 'APPROVED' ORDER BY c.created_at DESC LIMIT 6";
